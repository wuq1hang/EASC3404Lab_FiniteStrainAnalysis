...
We will discuss the solutions to Question 1 and 2 in class. Question 3 

\section*{Question 1}
We have derived the deformation gradient tensor $\bm{F}$ for the pure shear deformation in class.
...
the sinistral simple shear deformation with increasing shear strain, 
$\theta = $ \SI{30}{\degree} and \SI{60}{\degree}.
...
Plug the $\bm{F}$ you computed into \texttt{homogeneous\_strain.ipynb}. 
Experiment with different $\gamma$ values, can the modelled strain ellipsoids 
produce the similar deformation pattern as the shear zone in the figure?
\begin{figure}[hbpt!]
...
\begin{enumerate}
  \item Create two shear strain profiles, A and B, across the shear zone. 
    On transparent paper, mark at least 8 oblique fabrics along each profile and 
  ...
  \item Average the measurements along both profiles and input the values into 
    the notebook file \texttt{heterogeneous\_strain.ipynb} to 
    construct a spatially varying shear strain $\gamma$.
  ...
  \item Compute the overall displacement $d$ (\unit{\cm}) of the shear zone along each profile
    ...
    (Hint: consider using \texttt{scipy.integrate.simpson} )
